...
func Attach[T any, K any](ctx context.Context, db Queryer, row *T, relation string, ids ...K) error {
	...
	p, _, res, ownerKey, err := joinWriteTarget("Attach", db, row, relation)
	if err != nil {
		return err
	}
...
func Detach[T any, K any](ctx context.Context, db Queryer, row *T, relation string, ids ...K) error {
	...
	p, _, res, ownerKey, err := joinWriteTarget("Detach", db, row, relation)
	if err != nil {
		return err
	}
	if d := db.gram().d; !d.caps().mutations {
		return unsupportedf(
			"rio: Detach is not supported on %s "+
				"(join-table DELETE is an asynchronous mutation); use rio.Exec",
			d.name(),
		)
	}
...
// selectJoinRefs reads and locks the current join keys for SyncRelation.
func selectJoinRefs(
	ctx context.Context,
	tx *Tx,
	p *plan,
...
func joinWriteTarget[T any](op string, db Queryer, row *T, relation string) (*plan, *relField, *resolvedRel, any, error) {
	...
	if rel.kind != relManyToMany {
		return nil, nil, nil, nil, fmt.Errorf(
			"rio: %s handles ManyToMany relations; "+
				"a %s child moves by updating its foreign key",
			op,
			rel.kind,
		)
	}
	...
	rv, err := rowValue(op, row)
	if err != nil {
		return nil, nil, nil, nil, err
	}
	kv := rv.FieldByIndex(res.ref.index)
	if kv.IsZero() {
		return nil, nil, nil, nil, fmt.Errorf("rio: %s needs %s's primary key to be set", op, p.structName)
	}
