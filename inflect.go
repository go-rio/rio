...
func snakeCase(name string) string {
	...
	for i, c := range kept {
		...
		atBoundary := len(out) == 0 || out[len(out)-1] == '_'
		if startsWord(kept, i) && !atBoundary {
			out = append(out, '_')
		}
...
func startsWord(kept []byte, i int) bool {
	...
	nextLower := i+1 < len(kept) && isASCIILower(kept[i+1])
	return isASCIIUpper(prev) && nextLower
}
...
func pluralNoun(noun string) string {
	...
	n := len(noun)
	consonantY := n >= 2 && noun[n-1] == 'y' && isConsonant(noun[n-2])
	if consonantY {
		return noun[:n-1] + "ies"
	}
	sibilant := strings.HasSuffix(noun, "s") || strings.HasSuffix(noun, "x") ||
